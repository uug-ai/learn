...
selected on a per-tile basis with the SD/HD toggle:

- **SD — MQTT snapshots.** The agent encodes a low-resolution JPEG and
  publishes it over MQTT (TCP). The browser subscribes over secure
  WebSockets (WSS), so no port forwarding is required.
- **HD — WebRTC.** The full-resolution video is sent over WebRTC. NAT
...
SD to HD, only the underlying stream component changes; the rest of the
UI (camera name, controls, badges) stays in place.

{{< figure src="hub-livestream.gif" alt="Low-resolution snapshots over MQTT and full-resolution video over WebRTC, decoded directly in the browser." caption="Low-resolution snapshots travel over MQTT, high-definition video over WebRTC — both decoded directly in the browser." class="stretch">}}
...
{{< figure src="hub-livestream-pagination.png" alt="The numbered pagination bar at the bottom of the Live view page." caption="Numbered pagination places a classic page-number bar at the bottom of the grid. Use the page-size selector to pick how many streams are decoded at once." class="stretch">}}
...
{{< figure src="hub-livestream-stream.png" alt="A Live view tile with the SD/HD switcher, the talk button, the mute button and the fullscreen button visible." caption="Hover a tile to reveal the SD/HD switcher (top), and the talk, mute and fullscreen controls (bottom). The blinking dot reports the health of the underlying stream." class="stretch">}}

- **SD / HD** — switches the tile between the MQTT snapshot transport
  (SD) and the WebRTC transport (HD). HD requires a Gold subscription or
  higher; when your plan does not include HD the button is shown
  disabled with a tooltip explaining the upgrade path. The little
  blinking dot next to the switcher reports the health of the running
  stream — hover it in HD mode to see the live WebRTC statistics
  (resolution, FPS, bitrate, codec, RTT, jitter, packets lost, …).
- **Talk** — only shown when the agent reports a back-channel (ONVIF
  audio output or a compatible camera) and the tile is in HD. Press and
  hold to send your microphone audio to the camera's speaker. The
  surrounding volume bar visualises the level you're sending.
- **Mute / Unmute** — only shown in HD. Toggles the audio track of the
...
{{< figure src="hub-livestream-empty.png" alt="The Live view empty-state placeholder shown when no device is connected to the account." caption="On a fresh account with no connected devices, the Live view collapses to a placeholder that points to the device-onboarding flow." class="stretch">}}
...
- **Below Gold** — only the SD (MQTT) transport is available. The HD
  toggle is shown disabled with a tooltip linking to the subscription
  page.
- **Gold or higher** — both SD and HD transports are available, along
  with two-way talk and the live WebRTC statistics.
...
2. Switch the tile to **SD** to check whether MQTT alone works. If SD
   works but HD does not, the issue is on the WebRTC path (firewall,
   STUN/TURN reachability).
3. Hover the blinking dot on an HD tile to read the live WebRTC stats.
