...
(motion, intrusion, ANPR, …). Every notification row exposes an
**Add Case** action that opens the same *New case* modal, pre-filled with
the recording the notification refers to. It is the fastest path to
archive a recording from an event you were notified about.
...
Confirm with **New case** in the bottom-right of the modal to create the
case and queue the recording for archiving.
...
- **Delete** — removes the case (visible to the reporter, admins and the
  account owner only).
...
- **Delete** — removes the case. On this page, the *Delete* action is
  restricted to account **admins** and the account **owner**.
