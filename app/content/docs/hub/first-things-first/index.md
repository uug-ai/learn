...
A simplified architecture is found below. In this architecture the Agents are installed at the edge, next to the video streams. Recordings are being generated by one or more Agents, and send to [Kerberos Vault](/vault/first-things-first), using [the account credentials](/vault/accounts) created in Kerberos Vault.
