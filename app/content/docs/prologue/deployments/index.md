...
- How should I deploy these so called, [Agents](/docs/agent/first-things-first/)?
- Do I setup a [Kerberos Vault](/docs/vault/first-things-first/) next to my [Agents](/docs/agent/first-things-first/) or in a managed cloud?
- Can I self-host [Kerberos Hub](/docs/hub/first-things-first/) or do I need to install it in managed cloud?
...
If you are starting with a basic deployment - for example for your home - then you probably prefer to have it rather simple. In this case you can host one or more [Agents](/docs/agent/first-things-first) on a compute of choice, in the network you desire.
...
[Agents](/docs/agent/first-things-first) store recordings on the host system. You might want to have a more elegant and centralised storage setup. Run [Kerberos Vault](/docs/vault/first-things-first/) next to your
[Agents](/docs/agent/first-things-first) and connect to an edge or cloud storage system such as S3, Minio, etc.
...
Similar to the basic installation, Kerberos Vault can be installed through [`docker`](https://github.com/kerberos-io/vault/tree/master/docker) and [`kubernetes`](https://github.com/kerberos-io/vault/tree/master/kubernetes). In this setup, [Agents](/docs/agent/first-things-first) are installed on a compute at the edge, next to a [Kerberos Vault](/docs/vault/first-things-first/).

The advantage with [Kerberos Vault](/docs/vault/first-things-first/) is that even if it's deployed at the edge, you can still target a cloud storage system like S3, GCP Storage, etc. Next to that the main advantage is speed, you typically setup an [Agents](/docs/agent/first-things-first) within 5 minutes, and a [Kerberos Vault](/docs/vault/first-things-first/) installation within 30 minutes using `docker compose`.
...
The huge benefit of this approach is that your [Agents](/docs/agent/first-things-first) are installed next to the camera infrastructure, and ideally in the same network. This will bring latency and data transfer to a minimum.
...
The [Kerberos Vault](/docs/vault/first-things-first/) is installed in the cloud together with some scalable cloud storage. The [Kerberos Hub](/docs/hub/first-things-first/) is installed in the same or other cloud as the [Kerberos Vault](/docs/vault/first-things-first/).

The benefit of this deployment is that you only need to install [Kerberos Vault](/docs/vault/first-things-first/) and [Kerberos Hub](/docs/hub/first-things-first/) once. Most of the work is setting up the [Agents](/docs/agent/first-things-first) at your customers and/or sites.

Another benefit is low latency and bandwidth consumption, as we have compute running at the edge for the [Agents](/docs/agent/first-things-first). Only data (recordings) that are relevant will be send over the network to [Kerberos Vault](/docs/vault/first-things-first/). A possible disadvantage of this setup is that you will require some hardware at the site, although the maintenance is low it does come with a cost; have a look at [Cloud setup](/docs/prologue/deployments/#cloud-setup) instead.

## Cloud setup

With this setup we are moving from a Hybrid setup to a complete Cloud approach. As described before, it might be a challenge to host hardware at the edge, as you or your customers don't want to invest in additional hardware. Therefore it might be an option to move your [Agents](/docs/agent/first-things-first) to the cloud, and leverage a secure connection between the cameras at the edge, and the [Agents](/docs/agent/first-things-first) in the cloud.
...
The main advantage is here, is that you'll avoid any extra hardware costs on site. On the otherhand you'll need a secure connection, which might already be available, to setup a remote connection between the camera streams at the edge and the [Agents](/docs/agent/first-things-first) in the cloud.
...
As described above you might mix a Hybrid and Cloud setup, in the end you decide where to host your [Agents](/docs/agent/first-things-first). Within the SAAS setup, you'll utilise our [Kerberos Hub SAAS](/docs/hub/first-things-first/) edition, and connect your [Agents](/docs/agent/first-things-first) and [Kerberos Vault](/docs/vault/first-things-first/).
...
The main advantage of this setup is that you have full control over your [Agents](/docs/agent/first-things-first) and [Kerberos Vault](/docs/vault/first-things-first/), but consult the Kerberos.io team for visualizing your video landscape through [our Kerberos Hub SAAS](/docs/hub/first-things-first/) edition.

This means that you, and only you, own the data and at the same time doesn't have to maintain, install and configure [Kerberos Hub](/docs/hub/first-things-first/).

## Chained setup

If you need more and better redundancy then the Chained setup might be of interest. In this setup we move data (recordings) from one [Kerberos Vault](/docs/vault/first-things-first/) to another [Kerberos Vault](/docs/vault/first-things-first/).
...
The advantage of [chaining (or forwarding)](/docs/vault/forwarding/) is that you create a cache between two environments. Shown on the architecture above data is first stored at an edge [Kerberos Vault](/docs/vault/first-things-first/), and synchronised with a cloud [Kerberos Vault](/docs/vault/first-things-first/). When the connection goes down between the two [Kerberos Vaults](/docs/vault/first-things-first/), the data will still be stored in the edge [Kerberos Vault](/docs/vault/first-things-first/), and synced to the cloud [Kerberos Vault](/docs/vault/first-things-first/) once the connection is back up.

Another advantage is that [chaining (or forwarding)](/docs/vault/forwarding/) can be configured in different modes, whereas the previously mentioned advantage is illustrating `continuous` forwarding, it's also possible to have `ondemand` forwarding. In the latter, the user or administrators can decide programmatically or through our [Kerberos Hub](/docs/hub/first-things-first/) which recordings needs to be synchronised to the cloud [Kerberos Vault](/docs/vault/first-things-first/). This allows us to only send the information we prefer to our cloud [Kerberos Vault](/docs/vault/first-things-first/).
